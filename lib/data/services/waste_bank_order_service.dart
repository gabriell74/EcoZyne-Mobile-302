...
class WasteBankOrderService {
  ...
  Future<Map<String, dynamic>> acceptOrder(int orderId) async {
    try {
      final response = await _dio.put("/waste-bank/orders/$orderId/accept");
      ...
      return {
        "success": false,
        "message": response.data["message"] ?? "Gagal menerima pesanan",
        "connected": true,
      };
    } on DioException catch (e) {
      if (e.response != null) {
        return {
          "success": false,
          "message": e.response?.data["message"] ?? "Gagal menerima pesanan",
          "connected": true,
        };
      }
  ...
  Future<Map<String, dynamic>> completeOrder(int orderId) async {
    try {
      final response = await _dio.put("/waste-bank/orders/$orderId/complete");
      ...
      return {
        "success": false,
        "message": response.data["message"] ?? "Gagal menandai pesanan selesai",
        "connected": true,
      };
    } on DioException catch (e) {
      if (e.response != null) {
        return {
          "success": false,
          "message": e.response?.data["message"] ?? "Gagal menandai pesanan selesai",
          "connected": true,
        };
      }
